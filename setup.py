...
def get_author(package):
    ...
    return re.search("^__author__ = ['\"]([^'\"]+)['\"]", init_py,
                     re.MULTILINE).group(1)
...
def get_email(package):
    ...
    return re.search("^__email__ = ['\"]([^'\"]+)['\"]", init_py,
                     re.MULTILINE).group(1)
...
setup(
    ...
    description='slack API wrapper using tapioca',
    long_description=readme,
    author=get_author(package),
    author_email=get_email(package),
    url='https://github.com/humrochagf/tapioca-slack',
    packages=[
        'tapioca_slack',
    ],
    package_dir={'tapioca_slack': 'tapioca_slack'},
